...
def finetune_dhs_resnet50(parser):
    ...
    # extracted features
    ##################################################
    train_features, aug_features, val_features = get_extracted_features(DIRS)
    ...
    datf_aug = DHSFeatures(aug_features)
    aug_loader = DataLoader(datf_aug, batch_size=128, shuffle=True,num_workers=0)
    ...
    for j in range(args.n_epoch):
        ...
        for i,(x,y0) in enumerate(train_loader):
            ...
            # ################## aug #################
            # for k,(x,y0) in enumerate(aug_loader):
            #     x = x.to(device=device).to(torch.float) # yes it will always be shuffled, no worries
            #     y0 = y0.to(device=device).to(torch.float)
            #     y = nnhead(x)
...
def get_extracted_features(DIRS):
    ...
    return train_features, aug_features, val_features
...
def save_augmented_features_to_csv(DIRS):
    ...
    for i,(x,y0) in enumerate(val_loader):
        ...
        for j in range(4):    
            y_feature = net(x + torch.randn(x.shape).to(device=device))
            y_feature = y_feature + torch.randn(y_feature.shape).to(device=device)
    ...
    df.to_csv(DIRS['EXTRACTED_FEATURES_DIR.aug'], index=False)