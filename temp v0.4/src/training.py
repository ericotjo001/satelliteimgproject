...
def train_dhs_resnet18(parser:argparse.ArgumentParser) -> None:
    print('train_dhs_resnet18...')
    ...
    ##################################################
    ckpt = slm.saveload_ResNet18sw(init_new=False, MODEL_DIR=DIRS['MODEL_DIR'])
    net = ResNet18sw()
    if ckpt['model_state_dict'] is not None:
        net.load_state_dict(ckpt['model_state_dict'])
    net = net.to(device=device)
    optimizer = optim.Adam(net.parameters(),  lr=args.learning_rate, betas=(0.99,0.999))
    ...
    dset = DHSdataset(dataname='dhs', split='train',classify=False, 
        transformseq=get_data_transformation_sequence(model='resnet18'))
    loader = DataLoader(dset, batch_size=args.b1, shuffle=shuffle, num_workers=0)

    fitting_dset = DHSdataset(dataname='dhs', split='train', 
        transformseq=get_data_transformation_sequence(model='resnet18_val'))
    ...
    for j in range(args.n_epoch):
        ...
        for i,(x,y0) in enumerate(loader):
            x,y0 = x.to(device=device).to(torch.float), y0.to(device=device).to(torch.float)
            y = net(x)
        ...
        ckpt, r2 = validation_dhs_resnet18(val_loader, dargs, DIRS)
...
def validation_dhs_resnet18(val_loader: DataLoader, 
    dargs:dict, DIRS:dict) -> dict:
    """
    reg: regression model
    """

    print('\nvalidation_dhs_resnet18()')
    ...
    with torch.no_grad():
        for i,(x,y0) in enumerate(val_loader):
            ...
            y.append(net(x).clone().detach().cpu().numpy()[0])
